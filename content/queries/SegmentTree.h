 ...
 * License: CC0
 * Source: folklore
 * Description: Segment tree with lazy assignment on ranges. As written it sums;
 * change conquer for a different operation, and remember lazy uses -1 as "no
 * pending assignment", so -1 cannot be a stored value.
 * Usage: SegTree st(a); st.update(l, r, v); st.query(l, r);
